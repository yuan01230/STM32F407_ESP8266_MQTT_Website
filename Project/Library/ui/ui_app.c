 ...
 * 说明：
 * - 本文件是 UI 子系统的调度入口。
 * - 主要职责是根据当前页面状态，把按键分发到主页面、列表页、查看页等模块。
 * - 这里不负责具体绘制细节，绘制由 ui_main/ui_list/ui_view 等子模块完成。
 */
 ...
 * 该函数由主循环周期调用，负责两类工作：
 * 1. 非主页面下的无操作超时返回；
 * 2. 按秒刷新主页面运行时间或列表页倒计时。
 *
 * 这里故意不做高频整页重绘，而是只在“秒变化”时更新，
 * 这样可以减少 LCD 闪烁和无意义的重复绘制。
 */
void UI_Tick(void)
{
    uint32_t now_sec = UI_GetCurrentSeconds();
 ...
 * @details
 * 分发规则按“当前页面”决定：
 * - 主页面：保留原有颜色切换、创建文件、字库导入逻辑；
 * - 列表页：负责上下移动、进入目录、打开文件；
 * - 查看页：负责返回列表页。
 *
 * 这里的核心目的是把“页面行为”集中管理，避免按键逻辑分散在各处。
 */
