...
/**
 * @brief 读取指定 LED 当前是否处于点亮状态。
 * @param led LED 编号。
 * @return uint8_t 1=点亮，0=熄灭或编号非法。
 * @note
 * - gpio_device_read() 会根据 BOARD_LED_CONFIG_TABLE 中的有效电平配置做转换。
 * - 上层不需要关心 LED 是高电平有效还是低电平有效。
 * - 当前 MQTT 状态回传会调用该函数，把 LED 状态发布到 stm32/status。
 */
